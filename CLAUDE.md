...
- **Multi-community support** — the data model (`books`, `users` collections) and UI copy (e.g. the WhatsApp message text hardcodes "ספריית ניר עוז" / Nir Oz library) currently assume a single community. Multi-community support is a real future direction, so avoid further hardcoding single-community assumptions where reasonably avoidable in new work.
